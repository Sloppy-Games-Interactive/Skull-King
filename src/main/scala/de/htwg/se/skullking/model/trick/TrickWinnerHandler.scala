...
trait WinnerHandler {
  def handle(t: Trick): Option[Player]
}
...
class winnerAllEscapeWinnerHandler extends WinnerHandler {
  override def handle(t: Trick): Option[Player] = t.stack match {
...
class winnerSpecialWinnerHandler extends WinnerHandler {
  override def handle(t: Trick): Option[Player] = t.stack.filter((c, p) => c.suit != Suit.Escape) match {
